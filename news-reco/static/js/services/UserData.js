app.factory('userData', function($resource, $q, $cookies){
	return {
		login: function(data) {
			var resource = $resource(SOURCE + '/api/users/login');
			var deferred = $q.defer();
			resource.save({method: 'POST'}, {username: data.username, password: data.password},
				function(user) {
		...
		register: function(data) {
					...
					var resource = $resource(SOURCE + '/api/users/register');
					var deferred = $q.defer();
